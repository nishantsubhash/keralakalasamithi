...
export default function KksCard({ imageSlot, title, summary, actionSlot, meta, className = "" }: KksCardProps) {
  return (
    <div className={`bg-paper rounded-2xl shadow-md hover:shadow-lg transition border border-transparent hover:border-brand/30 overflow-hidden flex flex-col ${className}`.trim()}>
      {imageSlot && <div className="h-40 md:h-48 bg-gray-200 rounded-xl mb-4 overflow-hidden">{imageSlot}</div>}
      <div className="text-xl font-semibold text-palm">{title}</div>
      <div className="text-base md:text-lg text-gray-600 leading-relaxed mt-2">{summary}</div>
